...
class PEAR_Installer extends PEAR_Downloader
{
    ...
    function _installFile($file, $atts, $tmp_path, $options)
    {
        ...
        if (empty($this->_options['register.blade.php-only']) &&
     ...
     * - register.blade.php-only : update registry but don't install files
    ...
    function install($pkgfile, $options = array())
    {
        ...
        if ($test && empty($options['register.blade.php-only'])) {
        ...
        if (empty($options['register.blade.php-only']) && !is_dir($php_dir)) {
        ...
        $dirtree = (empty($options['register.blade.php-only']) && $p) ? $p->getDirTree() : false;
     ...
     *              - register.blade.php-only : update registry but don't remove files
    ...
    function uninstall($package, $options = array())
    {
        ...
        if (empty($options['register.blade.php-only'])) {
            // {{{ Delete the files
            $this->startFileTransaction();
            PEAR::pushErrorHandling(PEAR_ERROR_RETURN);
